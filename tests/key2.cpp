...
int main()
{
    ...
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<uint8_t> dist(0, rubik::action::G2.size() - 1);

    rubik::compact_state test;
    uint32_t key = rubik::thistlethwaite::key12(test);
    ...
    {
        test.twist(rubik::action::G2[dist(gen)]);
        key = rubik::thistlethwaite::key12(test);
